...
VARIABLE LABELS
  ...
  D51Q_N 'Trust businessmen NORM  J4_perceived_corr_judiciary
  J2_functional_subordination
  J5_judiciary_trust
  J3_case_rigging
  judical_average
  /STATISTICS=MEAN STDDEV MIN MAX.

* Strategic Capture and Contextual Indicators

COMPUTE foreign_influence = MEAN.1(
  SC1A_N, SC1B_N, SC1C_N, SC1D_N,
  SC2A_N, SC2B_N, SC2C_N, SC2D_N, SC2E_N, SC2F_N
).

COMPUTE elite_loyalty = SC3_N.

COMPUTE structural_barriers = SC7_3.

EXECUTE.

COMPUTE Strategic_Capture=MEAN.1(foreign_influence, elite_loyalty, structural_barriers )

VARIABLE LABELS
  foreign_influence "Foreign influence (external capture mechanisms)"
  elite_loyalty "Elite loyalty to foreign actors"
  structural_barriers "Structural barriers to independence".
...
* Preserved_corruption

COMPUTE corruption_pressure = MEAN.1(A12A_N, A12B_N).

COMPUTE victimization = MEAN.1(A13A_N, A13B_N, A13C_N).

COMPUTE social_norms = MEAN.1(A8_N, A15_N).

COMPUTE inst_trust_civsoc_corr = MEAN.1(
  A4A_N, A4B_N, A4C_N, A4J_N, A4Q_N, A4R_N, A4U_N, A4V_N
).

COMPUTE inst_trust_state_capture_corr = MEAN.1(
  A4D_N, A4E_N, A4F_N, A4K_N, A4L_N, A4N_N, A4P_N, A4T_N, A4W_N
).

EXECUTE.

COMPUTE PrC_average = MEAN.1(
  corruption_pressure , victimization, social_norms, inst_trust_civsoc_corr ,  inst_trust_state_capture_corr ).

VARIABLE LABELS
  corruption_pressure "Corruption pressure (requests/expectations)"
  victimization "Victimization (actual corrupt transactions)"
  social_norms "Social norms toward corruption"
  inst_trust_civsoc_corr "Perceived corruption in civil society actors"
  inst_trust_state_capture_corr "Perceived corruption in state capture actors".
...
DESCRIPTIVES VARIABLES=
  ...
  victimization
  social_norms
  inst_trust_civsoc_corr
  inst_trust_state_capture_corr
  PrC_average
/STATISTICS=MEAN STDDEV MIN MAX.
...
FACTOR 
  /VARIABLES L1_consultation_exclusion L2_ria_manipulation L3_transparency_resistance 
  /MISSING LISTWISE 
  /ANALYSIS L1_consultation_exclusion L2_ria_manipulation L3_transparency_resistance 
...
FACTOR 
  /VARIABLES   E1_procurement_concentration E2_admin_particularism E3_impunity
  /MISSING LISTWISE 
  /ANALYSIS  E1_procurement_concentration E2_admin_particularism E3_impunity
  /PRINT INITIAL KMO EXTRACTION 
  /CRITERIA FACTORS(1) ITERATE(25) 
  /EXTRACTION ML 
  /ROTATION NOROTATE 
...
FACTOR 
  /VARIABLES   J1_appointment_capture J4_perceived_corr_judiciary J2_functional_subordination J5_judiciary_trust J3_case_rigging
  /MISSING LISTWISE 
  /ANALYSIS  J1_appointment_capture J4_perceived_corr_judiciary J2_functional_subordination J5_judiciary_trust J3_case_rigging
  /PRINT INITIAL KMO EXTRACTION 
  /CRITERIA FACTORS(1) ITERATE(25) 
  /EXTRACTION ML 
  /ROTATION NOROTATE 
 ...
 FACTOR 
  /VARIABLES foreign_influence elite_loyalty structural_barriers
  /MISSING LISTWISE 
  /ANALYSIS  foreign_influence elite_loyalty structural_barriers
  /PRINT INITIAL KMO EXTRACTION 
  /CRITERIA FACTORS(1) ITERATE(25) 
  /EXTRACTION ML 
  /ROTATION NOROTATE 
...
FACTOR 
  /VARIABLES corruption_pressure victimization social_norms inst_trust_civsoc_corr inst_trust_state_capture_corr
  /MISSING LISTWISE 
  /ANALYSIS  corruption_pressure victimization social_norms inst_trust_civsoc_corr inst_trust_state_capture_corr
  /PRINT INITIAL KMO EXTRACTION 
  /CRITERIA FACTORS(1) ITERATE(25) 
  /EXTRACTION ML 
  /ROTATION NOROTATE 
...
COMPUTE L1_consultation_exclusion = L_PROC_A_N.
COMPUTE L2_ria_manipulation = MEAN.1(L_PROC_B_N, L_PROC_C_N).
COMPUTE L3_transparency_resistance = MEAN.1(
  A9A_N, A9B_N, A9C_N, A9D_N,
  A10A_N, A10B_N, A10C_N, A10D_N
).

VARIABLE LABELS
  L1_consultation_exclusion "L1 Consultation Exclusion"
  L2_ria_manipulation "L2 RIA Manipulation"
  L3_transparency_resistance "L3 Transparency Resistance".

EXECUTE.

COMPUTE legislative_dimension_unweighted = MEAN.1(
  L1_consultation_exclusion,
  L2_ria_manipulation,
  L3_transparency_resistance
).

VARIABLE LABELS
  legislative_dimension_unweighted "Legislative Dimension Unweighted Mean".
...
DESCRIPTIVES VARIABLES=
  ...
  L2_ria_manipulation
  L3_transparency_resistance
  legislative_dimension_unweighted
  /STATISTICS=MEAN STDDEV MIN MAX.

* EXECUTIVE DIMENSION

COMPUTE E1_procurement_concentration = MEAN.1(E1_PART_B_N, SC4_N).

COMPUTE E2_admin_particularism = MEAN.1(E1_PART_A_N, E1_PART_C_N).

COMPUTE E3_impunity = MEAN.1(J5_CON_C_N, A4O_N, A4S_N).

EXECUTE.

COMPUTE Executive_unweighted = MEAN.1(E1_procurement_concentration, E2_admin_particularism, E3_impunity ).

EXECUTE.

VARIABLE LABELS
  E1_procurement_concentration "E1 Procurement concentration"
  E2_admin_particularism "E2 Administrative particularism"
  E3_impunity "E3 Impunity (elite-level corruption)".

EXECUTE.

DESCRIPTIVES VARIABLES=
  E1_procurement_concentration
  E2_admin_particularism
  E3_impunity
  Executive_unweighted
  /STATISTICS=MEAN STDDEV MIN MAX.

* Judicial Dimension: Control and Immunity

COMPUTE J1_appointment_capture = J5_CON_A_N.
COMPUTE J4_perceived_corr_judiciary = MEAN.1(A4G_N, A4H_N, A4I_N).
COMPUTE J2_functional_subordination = J6_RES_N.
COMPUTE J5_judiciary_trust = MEAN.1(D51G_N, D51H_N, D51I_N).
COMPUTE J3_case_rigging = J5_CON_B_N.
EXECUTE.

VARIABLE LABELS
  J1_appointment_capture "J1 Appointment capture"
  J4_perceived_corr_judiciary "J4 Perceived corruption in judiciary"
  J2_functional_subordination "J2 Functional subordination"
  J5_judiciary_trust "J5 Judiciary trust"
  J3_case_rigging "J3 Case rigging".

EXECUTE.

COMPUTE judical_average = MEAN.1( J1_appointment_capture, J4_perceived_corr_judiciary, J2_functional_subordination, J5_judiciary_trust,  J3_case_rigging). 
EXECUTE.

DESCRIPTIVES VARIABLES=
  J1_appointment_capture
